...
(function(){
  ...
  if(volumeSlider){
    ...
    function setVolumeFromRaw(raw){
      ...
      // persist volume
      try{ localStorage.setItem('mibb_volume', String(Math.round(v*100))); }catch(e){}
  ...
  // Audio toggle (play/pause) and persisted volume
  if(bgAudio){
    // load persisted volume if present
    try{
      const saved = localStorage.getItem('mibb_volume');
      if(saved !== null && volumeSlider){ const v = Number(saved); bgAudio.volume = v/100; volumeSlider.value = v; }
    }catch(e){}
  }
  if(audioToggle && bgAudio){
    function updateAudioToggle(){ audioToggle.textContent = (!bgAudio.paused && !bgAudio.muted) ? '⏸' : '⏵'; audioToggle.setAttribute('aria-pressed', (!bgAudio.paused && !bgAudio.muted).toString()); }
    audioToggle.addEventListener('click', ()=>{
      if(bgAudio.paused){ bgAudio.play().then(()=>{ musicPlaying = true; updateAudioToggle(); }).catch(()=>{}); }
      else { bgAudio.pause(); musicPlaying = false; updateAudioToggle(); }
    });
    // reflect state changes
    bgAudio.addEventListener('play', updateAudioToggle);
    bgAudio.addEventListener('pause', updateAudioToggle);
    updateAudioToggle();
  }
  ...
  function showLightbox(index){
    ...
    currentIndex = index;
    // focus-trap: save last focused element and move focus into lightbox
    try{
      lightbox._lastActive = document.activeElement;
      if(closeBtn && typeof closeBtn.focus === 'function') closeBtn.focus();
      lightbox._keyHandler = function(e){ if(e.key !== 'Tab') return; const focusable = lightbox.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'); if(!focusable || focusable.length===0) return; const first = focusable[0]; const last = focusable[focusable.length-1]; if(e.shiftKey){ if(document.activeElement === first){ e.preventDefault(); last.focus(); } } else { if(document.activeElement === last){ e.preventDefault(); first.focus(); } } };
  ...
  function closeLightbox(){
    ...
    try{
      if(lightbox._keyHandler) document.removeEventListener('keydown', lightbox._keyHandler);
      if(lightbox._lastActive && typeof lightbox._lastActive.focus === 'function') lightbox._lastActive.focus();
      lightbox.removeAttribute('aria-modal');
    }catch(e){}
  }

  gallery.forEach((el, i)=>{ 
    // show thumbnail previews for videos (either <video> elements or imgs marked with data-type="video")
    if(el.tagName === 'VIDEO' || (el.tagName === 'IMG' && el.dataset && el.dataset.type === 'video')){
      try{
  ...
  if(effectsToggle && effectsPanel){
    // initialize aria-expanded for toggle
    try{ if(!effectsToggle.hasAttribute('aria-expanded')) effectsToggle.setAttribute('aria-expanded', 'false'); }catch(e){}
    effectsToggle.addEventListener('click', ()=>{
      const hidden = effectsPanel.getAttribute('aria-hidden') === 'true';
      effectsPanel.setAttribute('aria-hidden', hidden ? 'false' : 'true');
      try{ effectsToggle.setAttribute('aria-expanded', (!hidden).toString()); }catch(e){}
    });
    // delegate clicks
